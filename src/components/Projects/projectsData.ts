export const projects = [
  {
    title: 'IntelliBot',
    description: 'An AI-driven virtual assistant that optimizes backend services for improved customer support efficiency. Utilizes advanced AI models and cloud infrastructure for reliable performance.',
    image: 'https://imgur.com/UZqEmh7?auto=format&fit=crop&q=80&w=800',
    tech: ['Dart', 'Flutter', 'AWS EC2', 'EMR', 'OpenAI', 'Gemini', 'StabilityAI'],
    github: 'https://github.com/prakharnag/IntelliBot',
    achievements: [
      'Improved customer support efficiency by 50%',
      'Processed large datasets using AWS EC2 and EMR',
      'Implemented robust error handling with automated testing'
    ]
  },
  {
    title: 'IntelliQuery',
    description: 'A semantic search application leveraging AI-driven vector search to enhance search relevance. Built with scalability and performance in mind using modern cloud technologies.',
    image: 'https://imgur.com/iJxnu3s?auto=format&fit=crop&q=80&w=800',
    tech: ['Python', 'Docker', 'AWS EC2', 'Elasticsearch', 'OpenAI'],
    github: 'https://github.com/prakharnag/IntelliQuery',
    achievements: [
      'Improved search relevance by 30%',
      'Enhanced query performance by 50% using Elasticsearch',
      'Deployed scalable solution using Docker and AWS EC2'
    ]
  },
  {
    title: 'KitchenCraft',
    description: 'A mobile application for creating, managing, and searching recipes. Integrates with Spoonacular API to provide comprehensive recipe data and seamless user experience.',
    image: 'https://images.unsplash.com/photo-1556910103-1c02745aae4d?auto=format&fit=crop&q=80&w=800',
    tech: ['Dart', 'Flutter', 'Spoonacular API'],
    github: 'https://github.com/prakharnag/KitchenCraft-MobileApp',
    achievements: [
      'Improved search time by 40%',
      'Optimized system architecture',
      'Enhanced user experience with intuitive design'
    ]
  },
  {
    title: 'BookCrawlerX',
    description: 'An efficient web scraper for book data collection and analysis. Implements machine learning algorithms to optimize crawling and ensure accurate data retrieval.',
    image: 'https://images.unsplash.com/photo-1526243741027-444d633d7365?auto=format&fit=crop&q=80&w=800',
    tech: ['Python', 'Scrapy', 'Scikit-Learn', 'Flask', 'AWS EC2'],
    github: 'https://github.com/prakharnag/BookCrawlerX',
    achievements: [
      'Improved data accuracy and speed by 30%',
      'Implemented K-means clustering for optimization',
      'Scaled system using AWS EC2'
    ]
  },
  {
    title: 'Life on Blocks',
    description: 'A blockchain-based system for secure storage and processing of patient health records. Ensures data transparency and security in healthcare information management.',
    image: 'https://app.gemoo.com/share/image-annotation/730421449967788032?codeId=MpRaAZLao4nWo&origin=imageurlgenerator&card=730421449154093056?auto=format&fit=crop&q=80&w=800',
    tech: ['JavaScript', 'Python', 'Flask', 'MongoDB'],
    github: 'https://github.com/prakharnag/LifeOnBlocks',
    achievements: [
      'Secure storage of patient health records',
      'Enhanced data transparency',
      'Prevented fraudulent activities'
    ]
  }
];